...
export function convertFilesToTreeItems(files: {
  ...
  function covertNode(node: TreeNode, name?: string): TreeItem[] | TreeItem {
    ...
    for (const [key, value] of entries) {
      if (value === null) {
        ...
        // This a folder
        const subTress = covertNode(value, key);
        if (Array.isArray(subTress)) {
          children.push([key, ...subTress]);
        } else {
          children.push([key, subTress]);
        }
      }
